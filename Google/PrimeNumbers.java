...
public class PrimeNumbers {
    public static void main(String[] args) {
        System.out.println("Prime string is" + solution(0));
        System.out.println("Prime string is" + solution(3));
        System.out.println("Prime string is" + solution(100));
    }
    public static String solution(int i) {
        String s = generatePrimeString(i).toString();
        //System.out.println(s);
        return s.substring(i, i + 5);
    }

    private static StringBuilder generatePrimeString(int i) {
        boolean prime[] = new boolean[(i + 5) * (i + 5) + 1];
        for (int p = 2; p * p <= ((i + 5) * (i + 5)); p++) {
            if (!prime[p]) {
                for (int j = p * 2; j <= ((i + 5) * (i + 5)); j += p)
                    prime[j] = true;
            }
        }
